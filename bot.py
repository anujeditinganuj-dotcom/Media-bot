...
def build_ytdlp_command(url: str, temp_dir: Path, platform: str, quality: str = "best") -> list[str]:
    ...
    if platform == "youtube":
        common_flags = [
            # 2025 fix: android+tv_embedded most reliable; ios/web often blocked
            "--extractor-args",
            "youtube:player_client=android,tv_embedded,ios;skip=webpage,configs",
            "--no-check-certificates",
            "--retries", "10",
            "--fragment-retries", "10",
            "--retry-sleep", "exp=2",
            ...
            "--add-header", "User-Agent:com.google.android.youtube/19.29.37 (Linux; U; Android 14; en_US) gzip",
        ]
...
async def run_downloader(url: str, platform: str, quality: str = "best") -> tuple[list[Path], Path]:
    ...
    if rc != 0:
        ...
        # YouTube fallback chain: 3 attempts with different clients
        if platform == "youtube":
            ...
            # Attempt 2: tv_embedded only (bypasses age/region restrictions)
            logger.info("YouTube retry 2: tv_embedded client...")
            cmd2 = _make_yt_cmd("tv_embedded", fallback_fmt, is_audio)
            ...
            # Attempt 3: mweb (mobile web — different token path)
            logger.info("YouTube retry 3: mweb client...")
            cmd3 = _make_yt_cmd("mweb", fallback_fmt, is_audio)
            ...
            raise RuntimeError(
                f"❌ YouTube download failed after 3 attempts.\n\n"
                f"<b>Fix karo:</b> <code>pip install -U yt-dlp</code>\n\n"
                f"Error: {err3[:250] if err3 else err2[:250] if err2 else err[:250]}"
            )

        if platform == "facebook":
            logger.info("Facebook attempt 1 failed: %s", err[:200])
            logger.info("Facebook retry: using merged format...")
            alt_cmd2 = [
                "yt-dlp",
                "--no-check-certificates",
                "--retries", "5",
            ...
            raise RuntimeError(
                ...
                f"Please update yt-dlp: <code>yt-dlp -U</code>\n\n"
