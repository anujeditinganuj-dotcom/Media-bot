...
# =========================
# Pyrogram — MTProto upload (2GB native limit, no Bot API restriction)
# Bot API se file upload nahi hota 2GB ke baad — Pyrogram directly
# Telegram MTProto servers se baat karta hai, isliye 2GB tak kaam karta hai.
# =========================
try:
...
# Pyrogram client — lazily initialized in build_application()
# API_ID aur API_HASH env vars se liye jaate hain (my.telegram.org se milte hain)
...
TG_MAX_FILE_SIZE  = 2000 * 1024 * 1024   # 2 GB — Bot API hard limit
TG_PYROGRAM_LIMIT = 2000 * 1024 * 1024   # 2 GB — Pyrogram MTProto (same as Bot API for bots)
TG_STANDARD_LIMIT =   50 * 1024 * 1024   # 50 MB (inline play limit)

# Free user file size limit — src ke jaise 2GB cap
FREE_LIMIT_SIZE = 2 * 1024 * 1024 * 1024   # 2 GB

# Premium users list — env var se comma-separated user IDs
_premium_ids_raw = os.environ.get("PREMIUM_USER_IDS", "").strip()
PREMIUM_USER_IDS: set[int] = set()
if _premium_ids_raw:
    for _pid in _premium_ids_raw.split(","):
        try:
            PREMIUM_USER_IDS.add(int(_pid.strip()))
        except ValueError:
            pass


def is_premium_user(user_id: int) -> bool:
    """Check karo agar user premium hai (admin hamesha premium hota hai)."""
    return user_id == ADMIN_USER_ID or user_id in PREMIUM_USER_IDS
...
# =========================
async def get_pyrogram_client() -> "PyrogramClient | None":
    """
    Pyrogram client lazily initialize karo.
    API_ID aur API_HASH set hone chahiye (my.telegram.org se).
    Bot token se login hoga — user session nahi, bot session.
    """
    global _pyrogram_client
    if not PYROGRAM_AVAILABLE:
        return None
    if not API_ID or not API_HASH:
        logger.warning("Pyrogram unavailable: API_ID/API_HASH not set. Using Bot API (2GB limit).")
        return None
    if _pyrogram_client is None:
        try:
            _pyrogram_client = PyrogramClient(
                ...
                bot_token=BOT_TOKEN,
                in_memory=True,
                no_updates=True,   # sirf upload ke liye — updates receive mat karo
            )
            await _pyrogram_client.start()
            logger.info("✅ Pyrogram MTProto client started — 2GB upload enabled")
        except Exception as e:
            logger.error("Pyrogram client start failed: %s — falling back to Bot API", e)
            _pyrogram_client = None
    return _pyrogram_client


async def pyrogram_upload_file(
    chat_id: int,
    ...
    """
    Pyrogram se file upload karo — MTProto protocol, 2GB native support.
    Returns True on success, False on failure (caller Bot API fallback use karega).

    Progress callback signature: async def cb(current, total)
    """
    ...
    try:
        thumb_str = str(thumb_path) if (thumb_path and thumb_path.exists()) else None

        # caption length — Pyrogram 1024 char limit video/audio ke liye
        cap = caption[:1024]

        if ext in VIDEO_EXTS:
            _w    = int(video_info.get("width")    or 0) if video_info else 0
            _h    = int(video_info.get("height")   or 0) if video_info else 0
            _dur  = int(video_info.get("duration") or 0) if video_info else 0
            ...
            _artst   = (video_info.get("uploader") or video_info.get("channel") or "")[:64] if video_info else ""
            await client.send_audio(
                chat_id=chat_id,
                audio=str(file_path),
                caption=cap,
                duration=_dur or None,
                performer=_artst or None,
            ...
            # Unknown ext — document ke roop mein bhejo
        ...
        logger.error("Pyrogram upload failed for %s: %s", file_path.name, e)
        return False
...
async def send_media_files(
    ...
    for idx, file_path in enumerate(files):
        ...
        # Free user 2GB limit check — src ke jaise
        sender_id = message.from_user.id if message.from_user else 0
        if file_size > FREE_LIMIT_SIZE and not is_premium_user(sender_id):
            await message.reply_text(
                "⚠️ <b>File Size Exceeded</b>\n\n"
                f"Free users ke liye maximum file size <b>2GB</b> hai.\n"
                f"Is file ka size: <b>{format_size(file_size)}</b>\n\n"
                "💎 Premium users ke liye unlimited file size available hai.",
                parse_mode="HTML"
            )
            continue

        # ── Large file: GridFS mein store karo sirf agar explicitly configured ho ──
        if gridfs_mgr.available and file_size > TG_MAX_FILE_SIZE:
            await safe_edit_text(
                progress.status_message,
        ...
        # ── Thumbnail: video_info ka thumbnail use karo (sabhi file types pe) ──
        thumb_path: Path | None = None
        thumbnail_input = None   # open file object ya str URL

        if ext in VIDEO_EXTS or ext in AUDIO_EXTS:
            if fallback_thumb_url:
                # Step 1: URL se locally download karo — best quality
                try:
            ...
            if not thumbnail_input:
                logger.info("No thumbnail available for %s", file_path.name)
        ...
        # video_info se width/height/duration nikalo — Telegram player ke liye
        _vi_width    = int(video_info.get("width")    or 0) if video_info else 0
        _vi_height   = int(video_info.get("height")   or 0) if video_info else 0
        _vi_duration = int(video_info.get("duration") or 0) if video_info else 0
        # duration already set hai upar, lekin video_info wala prefer karo
        _send_duration = _vi_duration or duration

        try:
            # ── Pyrogram real-time progress callback ─────────────────────────
            _prog_ref  = progress
            _fs_ref    = file_size
            _fn_ref    = file_path.name
            _ql_ref    = quality_label
            _dur_ref   = _send_duration
            _us_ref    = upload_start
            _last_prog = [0.0]   # throttle: last edit timestamp

            async def _pyrogram_progress(current: int, total: int) -> None:
                """Pyrogram har chunk pe call karta hai — real bytes uploaded."""
                now = time.time()
                # 4 sec throttle — Telegram rate limit safe
                if now - _last_prog[0] < 4:
                    return
                _last_prog[0] = now
                elapsed = max(now - _us_ref, 0.1)
                pct     = min(int(current / max(total, 1) * 100), 99)
                speed   = current / elapsed
                eta     = int((total - current) / speed) if speed > 0 else 0
                await _prog_ref.set_uploading(
                    percent=pct,
                    filename=_fn_ref,
            ...
            # ── Try Pyrogram MTProto first (2GB native, real progress) ───────
            chat_id = message.chat_id
            msg_id  = message.message_id

            pyrogram_ok = await asyncio.wait_for(
                pyrogram_upload_file(
                    chat_id=chat_id,
                    ...
                    progress_cb=_pyrogram_progress,
                ),
                timeout=upload_timeout_secs,
            )

            # ── Bot API fallback (agar Pyrogram available nahi ya fail hua) ──
            if not pyrogram_ok:
                logger.info("Pyrogram unavailable/failed for %s — using Bot API fallback", file_path.name)

                async def _bot_api_send():
                    if ext in VIDEO_EXTS:
                        send_kwargs = dict(
                            caption=caption_doc,
                            supports_streaming=True,
                        ...
                        if _send_duration:
                            send_kwargs["duration"] = _send_duration
                        if _vi_width:
                            send_kwargs["width"] = _vi_width
                        if _vi_height:
                            send_kwargs["height"] = _vi_height
                        if thumbnail_input:
                ...
                await asyncio.wait_for(_bot_api_send(), timeout=upload_timeout_secs)
...
async def post_init(application: Application) -> None:
    ...
    # Pyrogram MTProto client — 2GB upload ke liye
    if PYROGRAM_AVAILABLE and API_ID and API_HASH:
        await get_pyrogram_client()
    else:
        logger.info("ℹ️ Pyrogram not configured — set API_ID + API_HASH env vars to enable 2GB MTProto uploads")
