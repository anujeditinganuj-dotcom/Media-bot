...
async def get_pyro_client() -> "PyroClient | None":
    ...
    async with lock:
        ...
        if _pyro_client is not None:
            try:
                await _pyro_client.get_me()  # connection alive check
                return _pyro_client
            except Exception:
...
# Playlist Store
# =========================
_playlist_store: dict[str, tuple[list[dict], str, float]] = {}

def store_playlist(videos: list[dict], title: str = "") -> str:
    _purge_expired_store(_playlist_store, _STORE_TTL)
    key = uuid.uuid4().hex[:8]
    _playlist_store[key] = (videos, title, time.time())
    return key

def get_playlist(key: str) -> tuple[list[dict], str] | None:
...
# ✅ FIX: is_youtube_playlist — playlist URL detect karo
def is_youtube_playlist(url: str) -> bool:
    try:
        ...
        # playlist?list=... ya watch?v=...&list=... dono handle karo
        has_list = "list" in qs
        # Pure playlist page (no video selected)
...
def search_results_keyboard(results: list[dict], search_key: str, page: int = 0, has_prev: bool = False) -> InlineKeyboardMarkup:
    ...
    # Row 2: buttons 6-10 (agar results 5 se zyada hain)
    row2 = []
    ...
    # Row 3: Prev / Next — page number bhi callback mein store karo
    nav_row = []
    if has_prev:
...
# ✅ NEW: Playlist keyboard — quality choose karo puri playlist ke liye
def playlist_keyboard(playlist_key: str) -> InlineKeyboardMarkup:
    ...
    # Filter buttons with too-long callback_data
    valid_buttons = []
    for row in buttons:
...
# =========================
# ✅ FIX: build_ytdlp_command — improved format strings, no dash/hls skip
# =========================
def build_ytdlp_command(url: str, temp_dir: Path, platform: str, quality: str = "best") -> list[str]:
    ...
    if quality == "audio_only":
        ...
        # ✅ FIX: Proper format strings with multiple fallbacks
        if quality == "best":
            fmt = (
...
# =========================
# ✅ NEW: build_ytdlp_playlist_command — playlist download
# =========================
...
# =========================
# ✅ FIX: build_ytdlp_info_command — ios+web clients
# =========================
...
# ✅ NEW: build_ytdlp_playlist_info_command — playlist metadata fetch
...
async def _search_youtube_via_api(query: str, max_results: int = 10, page: int = 0) -> list[dict]:
    ...
    try:
        ...
        # Page > 0 ke liye pageToken fetch karo
        if page > 0:
            # Pehle page tokens collect karo
...
async def search_youtube(query: str, max_results: int = 10, page: int = 0) -> list[dict]:
    # page * max_results skip karke agle results fetch karo
        ...
        if stdout:
            for line in stdout.decode(errors="replace").strip().split("\n"):
                ...
                try:
                    ...
                    continue
        if results:
            # Page ke hisaab se results slice karo
...
def build_search_results_text(query: str, results: list[dict], page: int = 0) -> str:
    ...
    for i, r in enumerate(results, 1):
        title = r["title"]  # Full title, no truncation
...
# =========================
# ✅ FIX: fetch_video_info — thumbnail properly set karo
# =========================
async def fetch_video_info(url: str, platform: str) -> dict | None:
    ...
    def _fix_thumbnail(info: dict) -> dict:
        """Thumbnail properly set karo from thumbnails list."""
...
# ✅ NEW: fetch_playlist_info — playlist ki saari videos ka info fetch karo
...
# =========================
# ✅ FIX: run_downloader — improved YouTube retry chain with better format strings
# =========================
async def run_downloader(url: str, platform: str, quality: str = "best") -> tuple[list[Path], Path]:
    ...
    if rc != 0:
        ...
        # ✅ FIX: YouTube 6-client retry chain with proper format strings
        if platform == "youtube":
            logger.info("YouTube attempt 1 failed: %s", err[:200])

            def _make_yt_cmd(clients: str, fmt: str, is_audio: bool = False) -> list:
                base = [
                    "yt-dlp",
            ...
            # Auto-update on signature error
            ...
            # Retry 2: ios only
            logger.info("YouTube retry 2: ios client...")
            cmd2 = _make_yt_cmd("ios", fallback_fmt, is_audio)
            ...
            # Retry 3: web
            logger.info("YouTube retry 3: web client...")
            cmd3 = _make_yt_cmd("web", fallback_fmt, is_audio)
            ...
            # Retry 4: android_vr
            logger.info("YouTube retry 4: android_vr client...")
            cmd4 = _make_yt_cmd("android_vr", fallback_fmt, is_audio)
            ...
            # Retry 5: tv_embedded
            logger.info("YouTube retry 5: tv_embedded client...")
            cmd5 = _make_yt_cmd("tv_embedded", fallback_fmt, is_audio)
            ...
            # Retry 6: mweb — last resort
            logger.info("YouTube retry 6: mweb (last resort)...")
            cmd6 = _make_yt_cmd("mweb", fallback_fmt, is_audio)
            stdout6, stderr6, rc6 = await _run_command(cmd6)
            files = collect_media_files(temp_dir)
            if files:
                return files, temp_dir
            err6 = (stderr6 or b"").decode(errors="replace").strip()

            final_err = err6 or err5 or err4 or err3 or err2 or err
            raise RuntimeError(
                f"❌ YouTube download failed after all attempts.\n\n"
                f"{_clean_error_message(final_err, 'youtube')}\n\n"
                f"💡 Tips:\n"
...
# ✅ NEW: run_playlist_downloader — playlist download karo
...
async def send_media_files(
    ...
    for idx, file_path in enumerate(files):
        ...
        # ✅ 2GB se badi file — skip karo, chhoti quality suggest karo
        ...
        try:
            async def _do_send():
                ...
                # ── Pyrogram upload — 2GB tak video as video ──
                if ext in VIDEO_EXTS or ext in AUDIO_EXTS:
                    pyro = await get_pyro_client()
                    if pyro is not None:
                        for pyro_attempt in range(3):  # 3 retries
                            try:
            ...
            if "413" in err_str or "Request Entity Too Large" in err_str:
                logger.warning("413 error for %s (%s).", file_path.name, format_size(file_size))
                raise RuntimeError(
...
# ✅ NEW: handle_youtube_playlist — playlist detect karke handle karo
async def handle_youtube_playlist(msg, context, url: str) -> None:
    """YouTube playlist URL ko handle karo — info fetch karke quality choose karne do."""
    ...
    playlist_key = store_playlist(videos, playlist_title)

    title_display = playlist_title or "YouTube Playlist"
    count = len(videos)

    # First few titles preview
    preview_lines = []
...
# ✅ NEW: handle_playlist_callback — playlist quality choose karne par download shuru karo
async def handle_playlist_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status = await msg.reply_text(
        ...
        f"⏳ Please wait — sari videos ek ek karke download aur send hongi...",
        parse_mode="HTML"
    )
    progress = StatusProgress(status)

    # Playlist URL reconstruct — pehli video ke url se playlist URL reconstruct nahi kar sakte
    # Isliye original URL store karna padega. Workaround: playlist_store mein url bhi store karo.
    # Abhi ke liye: hum videos individually download karte hain
    async with download_semaphore:
        temp_dir = None
        try:
            await progress.start_downloading()
            # videos list se pehla item ka URL use karke playlist download karte hain
            # Actually playlist_store mein original URL hona chahiye
            # Yahan hum _playlist_store mein url bhi store karte hain (see store_playlist fix below)
            playlist_url = _playlist_store.get(playlist_key + "_url", ("",))[0] if (playlist_key + "_url") in _playlist_store else ""

            # Fallback: videos se reconstruct
            if not playlist_url and videos:
                first_url = videos[0].get("url", "")
                # playlist id extract karo
                from urllib.parse import urlparse, parse_qs
                for v in videos:
                    vurl = v.get("url", "")
                    if "list=" in vurl:
...
async def post_init(application: Application) -> None:
    ...
    # Pyrogram client start karo agar API_ID aur API_HASH set hain
...
def build_application() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(handle_playlist_callback,       pattern=r"^pl\|"))   # ✅ NEW
    app.add_handler(CallbackQueryHandler(handle_playlist_callback,       pattern="^pl_cancel$"))  # ✅ NEW
