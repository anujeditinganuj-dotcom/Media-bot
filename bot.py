...
def build_ytdlp_command(url: str, temp_dir: Path, platform: str, quality: str = "best") -> list[str]:
    ...
    if platform == "youtube":
        common_flags = [
            ...
            "--extractor-args",
            "youtube:player_client=web_creator,android,tv_embedded;skip=webpage,configs",
            "--no-check-certificates",
            "--retries", "10",
            "--fragment-retries", "10",
            "--retry-sleep", "exp=2",
...
async def run_downloader(url: str, platform: str, quality: str = "best") -> tuple[list[Path], Path]:
    ...
    if rc != 0:
        ...
        # YouTube fallback chain: 4 attempts with different clients
        if platform == "youtube":
            ...
            # Attempt 2: web_creator (bypasses JS/signature requirement — 2026 fix)
            logger.info("YouTube retry 2: web_creator client...")
            cmd2 = _make_yt_cmd("web_creator", fallback_fmt, is_audio)
            ...
            # Attempt 3: tv_embedded only (bypasses age/region restrictions)
            ...
            raise RuntimeError(
                f"❌ YouTube download failed after 4 attempts.\n\n"
                f"<b>Fix karo:</b> <code>pip install -U yt-dlp</code>\n\n"
                f"Error: {err4[:250] if err4 else err3[:250] if err3 else err[:250]}"
            )
...
def auto_update_ytdlp():
    """Deploy hote hi yt-dlp latest version install karo."""
    try:
        logger.info("🔄 yt-dlp auto-update shuru ho raha hai...")
        ...
        if result.returncode == 0:
            ...
            logger.info("✅ yt-dlp updated: v%s", ver.stdout.strip())
        else:
