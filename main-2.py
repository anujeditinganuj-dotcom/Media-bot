...
# =========================
_search_store: dict[str, tuple[list[dict], str, float]] = {}
...
# =========================
# ✅ FIX: parse_format_sizes — 144p to 8K auto detect
# =========================
def parse_format_sizes(info: dict) -> dict[str, int]:
    """
    Dynamically capture ALL available video heights (144p to 8K) and
    audio-only formats. Returns dict like:
    {"4320p": 8_000_000_000, "2160p": 2_000_000_000, "1080p": 500_000_000,
     "720p": 200_000_000, "480p": 80_000_000, "360p": 40_000_000,
     "240p": 20_000_000, "144p": 10_000_000, "MP3 128kbps": 5_000_000}
    """
    sizes: dict[str, int] = {}
    ...
    # ── Video formats: every unique height ──
    for fmt in formats:
    ...
    # ── Audio-only: best bitrate pick karo ──
    best_audio_fs  = 0
    best_audio_abr = 0.0
    for fmt in formats:
        vcodec = (fmt.get("vcodec") or "none").lower()
...
# =========================
# ✅ FIX: quality_keyboard — auto detect all qualities 144p to 8K
# =========================
...
# =========================
# ✅ FIX: build_ytdlp_command — ios+web clients, improved format strings
# =========================
def build_ytdlp_command(url: str, temp_dir: Path, platform: str, quality: str = "best") -> list[str]:
    ...
    common_flags = []
    if platform == "youtube":
        # ✅ ios + web primary — 2026 mein sabse reliable, no JS/nsig issues
        # ✅ skip=webpage,configs HATA DIYA — format listing block kar raha tha
        common_flags = [
    ...
    if quality == "audio_only":
        ...
        # ✅ FIX: improved format strings with proper fallbacks
        if quality == "best":
            fmt = "bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best"
        elif re.match(r"^\d+p$", quality):
            h = quality[:-1]
            # Try mp4+m4a first, then any codec, then best available, then no height filter
            fmt = (
...
# =========================
# ✅ FIX: build_ytdlp_info_command — ios+web clients, playlist support
# =========================
def build_ytdlp_info_command(url: str, platform: str) -> list[str]:
    common_flags = []
    if platform == "youtube":
        # ✅ ios: no JS needed, returns full format list including 8K
        # ✅ playlist URLs ke liye --no-playlist hata diya (playlist link ho to sab aaye)
        ...
        if not is_playlist:
            common_flags.append("--no-playlist")
    elif platform == "facebook":
        common_flags = [
            "--no-check-certificates",
            "--socket-timeout", "30",
...
async def search_youtube(query: str, max_results: int = 8) -> list[dict]:
    ...
    # ✅ FIX: ios client use karo search mein bhi
    command = [
        "yt-dlp",
...
# =========================
# ✅ FIX: fetch_video_info — ios client + web fallback
# =========================
    ...
    try:
        ...
        if platform == "youtube":
            ...
            if p2.returncode == 0 and stdout2:
                ...
                if not info2.get("thumbnail") and info2.get("thumbnails"):
                    thumbs2 = info2["thumbnails"]
                    best2 = max(thumbs2, key=lambda t: (t.get("width") or 0) * (t.get("height") or 0), default=None)
                    if best2:
                        info2["thumbnail"] = best2.get("url", "")
...
# =========================
# ✅ FIX: run_downloader — 6-client retry chain for YouTube
# =========================
async def run_downloader(url: str, platform: str, quality: str = "best") -> tuple[list[Path], Path]:
    ...
    if rc != 0:
        ...
        # ✅ FIX: YouTube 6-client retry chain
        if platform == "youtube":
            ...
            if is_audio:
                fallback_fmt = "bestaudio[ext=m4a]/bestaudio/best"
            elif quality == "best":
                fallback_fmt = "bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best"
            elif re.match(r"^\d+p$", quality):
                h = quality[:-1]
                fallback_fmt = (
                    f"bestvideo[height<={h}][ext=mp4]+bestaudio[ext=m4a]"
